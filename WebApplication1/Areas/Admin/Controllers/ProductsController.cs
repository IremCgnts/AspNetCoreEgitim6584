        ...
        public ActionResult Create(Product collection, IFormFile? Image)
        {
            try
            {
                if (Image is not null)
                {
                    ...
                    using var stream = new FileStream(directory, FileMode.Create);// dosyanın seçildiği cihazdan sunucuya doğru bir veri akışı oluşturuyoruz FileSteam nesnesiyle
                    Image.CopyTo(stream);// dosyayı yukardaki ayaralar ile sunucuya kopyalıyoruz
